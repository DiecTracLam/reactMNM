  ...
  return (
    <div onClick={() => handleDetail(item.id)} className="grid__column-2-4">
      <a className="home-product-item">
        ...
        ></div>
        <h4 className="home-product-item-name">{item.productName}</h4>
        <div className="home-product-item-price">
          <span
            // style={{ textDecoration: isSale ? "" : "none" ,  }}
            className={`home-product-item-price-old ${isSale?"":"product-item-price-old_noSale"}`}
          >
            {formatPrice(item.price)}
          </span>
          {isSale && (
            <span className="home-product-item-price-new">
              {formatPrice(item.priceAfter)}
            </span>
          )}
        </div>
